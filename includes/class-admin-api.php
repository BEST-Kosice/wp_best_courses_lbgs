...
/**
 * Admin API to be used by Settings class.
 *
 * @see     Settings
...
class Admin_API {
        ...
        switch ( $field['type'] ) {
            ...
            case 'email':
                $html .= '<input id="' . esc_attr( $field['id'] ) . '" type="text" name="' . esc_attr( $option_name ) .
                         '" placeholder="' . esc_attr( $field['placeholder'] ) . '" value="' . esc_attr( $data ) .
                         '" />' . "\n";
                break;
            ...
            case 'checkbox_multi':
                foreach ( $field['options'] as $k => $v ) {
                    ...
                    $html .= '<label for="' . esc_attr( $field['id'] . '_' . $k ) .
                             '" class="checkbox_multi"><input type="checkbox" ' . checked( $checked, true, false ) .
                             ' name="' . esc_attr( $option_name ) . '[]" value="' . esc_attr( $k ) . '" id="' .
                             esc_attr( $field['id'] . '_' . $k ) . '" /> ' . $v . '</label> ';
                }
                break;
            ...
            case 'radio':
                foreach ( $field['options'] as $k => $v ) {
                    ...
                    $html .= '<label for="' . esc_attr( $field['id'] . '_' . $k ) . '"><input type="radio" ' .
                             ...
                             esc_attr( $k ) . '" id="' . esc_attr( $field['id'] . '_' . $k ) . '" /> ' . $v .
                             '</label> ';
                }
                break;
            ...
            case 'select':
                ...
                foreach ( $field['options'] as $k => $v ) {
                    ...
                    $html .= '<option ' . selected( $selected, true, false ) . ' value="' . esc_attr( $k ) . '">' . $v .
                             '</option>';
                }
                $html .= '</select> ';
                break;
            ...
            case 'image':
                ...
                $html .= '<input id="' . $option_name . '_button" type="button" data-uploader_title="' .
                         __( 'Upload an image', PLUGIN_NAME ) . '" data-uploader_button_text="' .
                         __( 'Use image', PLUGIN_NAME ) . '" class="image_upload_button button" value="' .
                         __( 'Upload new image', PLUGIN_NAME ) . '" />' . "\n";
                $html .= '<input id="' . $option_name .
                         '_delete" type="button" class="image_delete_button button" value="' .
                         __( 'Remove image', PLUGIN_NAME ) . '" />' . "\n";
                $html .= '<input id="' . $option_name . '" class="image_data_field" type="hidden" name="' .
                         $option_name . '" value="' . $data . '"/><br/>' . "\n";
                break;
    ...
    public function add_meta_box(
        $id = '', $title = '', $post_types = array(), $context = 'advanced', $priority = 'default',
        $callback_args = null
    ) {
        ...
        foreach ( $post_types as $post_type ) {
            add_meta_box( $id, $title, array( $this, 'meta_box_content' ), $post_type, $context, $priority,
                $callback_args );
        }
    }
        ...
        foreach ( $fields as $field ) {
            if ( isset( $_REQUEST[ $field['id'] ] ) ) {
                update_post_meta( $post_id, $field['id'],
                    $this->validate_field( $_REQUEST[ $field['id'] ], $field['type'] ) );
            } else {
                update_post_meta( $post_id, $field['id'], '' );
            }
        }
