    ...
    function LBGs_add_effects(color, secondaryColor, LBGs){
        ...
        for (var idx = 0; idx < LBGs.length; idx++){
            (function(lbg){
                ...
                var paper = lbg.paper;
                //add a text and a rectangle to represent the box containing the text
                var countrycode = lbg.node.id.substr(-2);
                var text = paper.text(bbox.x + 20, bbox.y + 30, lbg.node.id.replace(" " + countrycode, ", " + countryNames[countrycode]));
                /* 
                ...
                var rectangle = paper.rect(textbbox.x - 5, textbbox.y - 5, textbbox.width + 20, textbbox.height + 20, 3, 3);
                lbg.attr({fill: color});
                ...
                lbg.mouseover(function(){
                    lbg.animate({fill: secondaryColor, transform: 'translate('+(-bbox.cx*(2.5-1))+','+(-bbox.cy*(2.5-1))+') scale(2.5 2.5)'}, animationSpeed);
                    rectangle.attr({display: "block"});
                    text.attr({display: "block"});
                });
                //onmouseout make the dot small again and hide the box with the text
                lbg.mouseout(function(){
                    lbg.animate({fill: color, transform: 'translate('+(-bbox.cx*(1-1))+','+(-bbox.cy*(1-1))+') scale(1 1)'}, animationSpeed);
